...
static void block_swap_sl(int32_t *restrict arr, size_t m, size_t p, size_t ll) {
    int32_t tmp = arr[m];
    size_t init = m;
    size_t j = m;
    size_t nm = p - ll + 1;
...
static void block_swap(int32_t *restrict arr, size_t m, size_t r, size_t p) {
    ...
    else block_swap_sl(arr, m, p, ll);
}

static size_t partition(int32_t *restrict arr, size_t l, size_t r, size_t p_idx) {
    int32_t pivot_val = arr[p_idx];
    size_t i = l - 1; 
    size_t j = r;
    while (true) {
        while (true) { i++; if (i == j) return i; if (arr[i] >= pivot_val) break; }
        while (true) { j--; if (j == i) return i; if (arr[j] <= pivot_val) break; }
        swap(&arr[i], &arr[j]);
    }
}
...
static void downwave(int32_t *restrict arr, size_t start, size_t sorted_start, size_t end) {
