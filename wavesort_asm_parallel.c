...
// ============================================================================
extern void wave_sort(int32_t *arr, size_t n); // Serial ASM entry
...
static void downwave_par(int32_t *restrict arr, size_t start, size_t sorted_start, size_t end) {
    if (sorted_start == start) return;
    
    // Serial fallback for small chunks
    if (end - start < TASK_CUTOFF) {
        // We can call the serial ASM kernels recursively via C, or just rely on a pure C fallback.
        // For simplicity, let's just stick to the C logic calling ASM kernels.
    }
    ...
    if (m == sorted_start) {
        if (p == sorted_start) {
            if (sorted_start > 0) upwave_par(arr, start, sorted_start - 1);
            return;
        }
        if (p > 0) downwave_par(arr, start, sorted_start, p - 1);
        return;
    }
    ...
    if (p == sorted_start) {
        if (m > 0) upwave_par(arr, start, m - 1);
        upwave_par(arr, m + 1, end);
        return;
    }
...
int main(void) {
    ...
    int32_t *arr_par = (int32_t*)malloc(ARR_SIZE * sizeof(int32_t));
    
    if (!arr_base || !arr_qsort || !arr_asm || !arr_par) return 1;
    ...
    if (!is_sorted(arr_asm, ARR_SIZE)) printf("Serial ASM FAILED!\n");
    ...
    if (!is_sorted(arr_par, ARR_SIZE)) printf("Parallel C+ASM FAILED!\n");
